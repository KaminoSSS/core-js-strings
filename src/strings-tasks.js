...
function repeatString(str, times) {
  const correctTimes = times > 0 ? times : 0;
  return str.repeat(correctTimes);
}
...
function isPalindrome(str) {
  const cringeCharsArr = ['!', ',', '?', ' '];
  const cleanStr = str
    .split('')
    .filter((el) => !cringeCharsArr.includes(el))
    .map((el) => el.toLowerCase())
    .join('');
  const revStr = cleanStr.split('').reverse().join('');
  const result = cleanStr === revStr;
  return result;
}
...
function findLongestWord(sentence) {
  ...
  arr.forEach((el) => {
    if (biggestLength < el.length) {
      biggestLength = el.length;
      biggestWord = el;
    }
  });
  return biggestWord;
...
function encodeToRot13(str) {
  ...
  return strArr
    .map((letter) => {
      if (cringeCharsArr.includes(letter)) {
        return letter;
      }
      let letterCase;
      ...
      if (alphabet.length - alphabet.indexOf(letter.toLowerCase()) <= 13) {
        return caseFunc(
          letterCase,
          alphabet[
            13 - (alphabet.length - alphabet.indexOf(letter.toLowerCase()))
          ]
        );
      }
      return caseFunc(
        letterCase,
        alphabet[alphabet.indexOf(letter.toLowerCase()) + 13]
      );
    })
    .join('');
}
